...
def sync_session_inbound(session_id):
    """
    Pull external calendar changes into published DropCal events for a session.
    Delegates to factory.sync_session_inbound() for all logic.
    """
    try:
        result = factory.sync_session_inbound(request.user_id, session_id)
